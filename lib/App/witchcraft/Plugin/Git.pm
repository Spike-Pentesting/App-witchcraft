                ...
                or system("git reset --hard origin/master") != 0 )
            {
                ...
                notice(
                    __x(
                        ...
                        repository => App::witchcraft->instance->Config->param(
                            'GIT_REPOSITORY')
                    )
                );
            }
    ...
    $emitter->on(
        "stage_changes" => sub {
            ...
            foreach my $atom (@Atoms) {
                ...
                eval {
                    git::commit -m =>
                      __x( '[{atom}] automatically added/updated by witchcraft',
                        atom => $atom );
                };
                if ($@) {
                    send_report(
        ...
        "align_to" => sub {
            ...
            if ( $git_last_commit ne $last_commit ) {
                ...
                if (   detect_rebase($last_commit)
                    or invalid_commit($last_commit) )
                {
                    my $commit_ahead = App::witchcraft->instance->Config->param(
                        'GIT_REBASE_COMMIT_AHEAD')
                      // 2;    #defaults to penultimate commit
